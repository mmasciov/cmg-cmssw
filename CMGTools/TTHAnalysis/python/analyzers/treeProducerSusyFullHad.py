...
##    NTupleVariable("genQScale", lambda ev : ev.genQScale, help="Generator level binning quantity, QScale"),
    ...
    NTupleVariable("diffMetMht", lambda ev : ev.diffMetMht_30j, help="abs( vec(mht) - vec(met) ) - with jets and leptons"),
    NTupleVariable("deltaPhiMin", lambda ev : ev.deltaPhiMin_30j, help="minimal deltaPhi between the MET and the four leading jets with pt>40 and eta<2.4 and leptons (electrons ans muons with |eta|<2.5, pt > 10 GeV)"),
    ...
    NTupleVariable("ht_had", lambda ev : ev.htJet30j, help="H_{T} computed from only jets (with |eta|<2.5, pt > 30 GeV)"),
    NTupleVariable("mht_had_pt", lambda ev : ev.mhtJet30j, help="H_{T}^{miss} computed from only jets (with |eta|<2.5, pt > 30 GeV)"),
    NTupleVariable("mht_had_phi", lambda ev : ev.mhtPhiJet30j, help="H_{T}^{miss} #phi computed from only jets (with |eta|<2.5, pt > 30 GeV)"),
    NTupleVariable("diffMetMht_had", lambda ev : ev.diffMetMht_30j_had, help="abs( vec(mht) - vec(met) ) - only jets"),
    NTupleVariable("deltaPhiMin_had", lambda ev : ev.deltaPhiMin_30j_had, help="minimal deltaPhi between the MET and the four leading jets with pt>30 and eta<2.4"),
    ...
    NTupleVariable("gamma_ht", lambda ev : ev.gamma_htJet30j, help="H_{T} computed from only jets (with |eta|<2.5, pt > 30 GeV)"),
    #NTupleVariable("gamma_ht", lambda ev : ev.gamma_htJet40j, help="H_{T} computed from only jets (with |eta|<2.5, pt > 40 GeV)"),
    NTupleVariable("gamma_deltaPhiMin", lambda ev : ev.gamma_deltaPhiMin_30j_had, help="minimal deltaPhi between the MET and the four leading jets with pt>30 and eta<2.4"),
    NTupleVariable("gamma_diffMetMht", lambda ev : ev.gamma_diffMetMht_30j_had, help="abs( vec(mht) - vec(met) )"),
    NTupleVariable("gamma_mht_pt", lambda ev : ev.gamma_mhtJet30j, help="H_{T}^{miss} computed from jets (with |eta|<2.5, pt > 30 GeV) and leptons (electrons ans muons with |eta|<2.5, pt > 10 GeV)"),
    NTupleVariable("gamma_mht_phi", lambda ev : ev.gamma_mhtPhiJet30j, help="H_{T}^{miss} #phi computed from jets (with |eta|<2.5, pt > 30 GeV) and leptons (electrons ans muons with |eta|<2.5, pt > 10 GeV)"),
    ...
    # Zll variables
    ##--------------------------------------------------
    NTupleVariable("zll_deltaPhiMin", lambda ev : ev.zll_deltaPhiMin_30j, help="minimal deltaPhi between the zll MET and the four leading jets with pt>30 and eta<2.4"),
    NTupleVariable("zll_diffMetMht", lambda ev : ev.zll_diffMetMht_30j, help="abs( vec(mht) - vec(met) ) - only jets for mht, jets plus 2 leptons for met"),
    NTupleVariable("zll_mht_pt", lambda ev : ev.zll_mhtJet30j, help="H_{T}^{miss} computed from only jets (with |eta|<2.5, pt > 30 GeV)"),
    NTupleVariable("zll_mht_phi", lambda ev : ev.zll_mhtPhiJet30j, help="H_{T}^{miss} #phi computed from only jets (with |eta|<2.5, pt > 30 GeV)"),
    NTupleVariable("zll_met_pt", lambda ev : ev.zll_met_pt, help="E_{T}^{miss} computed from jets (with |eta|<2.5, pt > 30 GeV) + 2 leptons"),
    NTupleVariable("zll_met_phi", lambda ev : ev.zll_met_phi, help="E_{T}^{miss} #phi computed from jets (with |eta|<2.5, pt > 30 GeV) + 2 leptons"),
    NTupleVariable("zll_ht", lambda ev: ev.zll_ht_30j, float, help="H_{T} computed from only jets (with |eta|<2.5, pt > 30 GeV)"),
